...
- Using the instance `execute()` was actually given (previous fix) meant a concurrent execution that disables caching and cleans up its file could no longer stop an in-flight execution's later cache write, which would silently recreate the file that was just deleted. Disabling now marks the outgoing instance invalidated, and `execute()` checks that marker before every cache read/write.
- That invalidation marker only stopped a write from *starting*; a write that had already passed the check before cleanup ran could still complete its (debounced, up to ~100ms) disk write afterwards and recreate the just-deleted file. Cleanup now waits for a counter of in-flight writes on the outgoing instance to drain to zero before deleting the file, so an already-started write is guaranteed to land first.
